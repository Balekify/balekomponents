...
export default class Carousel extends React.Component {
  ...
  componentWillMount() {
    ...
    const gutter =
      slideToShow === 1
        ? 0
        : (this.props.gutter || option.gutter) /
          100 *
          slideWidth *
          100 /
          carouselWidth
  ...
  handleClickPrev() {
    if (!this.state.wait) {
      this.setState(prev => {
        const currentSlide =
          prev.currentSlide - prev.slideToScroll < 0
            ? 0
            : prev.currentSlide - prev.slideToScroll
  ...
  handleClickNext() {
    if (!this.state.wait) {
      this.setState(prev => {
        let currentSlide =
          prev.currentSlide + prev.slideToScroll < prev.nbrSlides
            ? prev.currentSlide + prev.slideToScroll
            : prev.nbrSlides - 1
  ...
  render() {
    ...
    const CarouselStyle = {
      ...
      width: `${carouselWidth}%`,
      transform: `translateX(-${(tile + gutter / slideToShow) *
        currentSlide}%)`,
    }

    const SlideStyle = { width: `${tileWidth}%`, marginRight: `${gutter}%` }

    const Dots = hideDots ? null : (
      <Dot
        nbrSlides={nbrSlides}
    ...
    children.map(item => {
      const li = (
        <li className="Slide" style={SlideStyle} key={shortid.generate()}>
          {item}
        </li>
      )
    ...
    return (
      <div className="Carousel">
        <Arrow handleClick={this.handleClickPrev} fade={!currentSlide} />
        <Arrow
